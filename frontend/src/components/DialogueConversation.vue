...
<template>
  ...
  >
    <div class="flex h-12 w-12 items-center justify-center rounded-full bg-muted text-lg">💬</div>
    <p class="text-sm">Выберите диалог слева</p>
  </div>

  <div v-else class="flex h-full flex-col">
    <div v-if="store.dialogue" class="flex items-center justify-between border-b bg-muted/40 px-5 py-3">
      <div class="flex items-center gap-3">
        <InitialsAvatar :name="store.dialogue.client.name" role="client" />
        <div>
          <div class="text-sm font-semibold">{{ store.dialogue.client.name }}</div>
          <div class="text-xs text-muted-foreground">Менеджер: {{ store.dialogue.manager.name }}</div>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-y-auto px-5 py-4">
      <div v-if="hasEarlier" class="mb-4 flex justify-center">
        <Button variant="outline" size="sm" :disabled="store.isLoadingMessages" @click="store.loadEarlierMessages()">
          {{ store.isLoadingMessages ? 'Загрузка…' : 'Загрузить ранние' }}
        </Button>
      </div>

      <div v-if="store.isLoadingMessages && !store.messages.length" class="flex flex-col gap-4">
        <Skeleton v-for="i in 5" :key="i" class="h-14 w-1/2" :class="i % 2 ? 'self-start' : 'self-end'" />
      </div>
      ...
      <div class="flex flex-col gap-4">
        ...
        >
          ...
          <div class="flex max-w-[72%] flex-col" :class="isManager(m) ? 'items-end' : 'items-start'">
