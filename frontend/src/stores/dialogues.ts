...
import type { DialogueListItem, Message, Pagination, Paginated } from '@/types/dialogue'
...
export const useDialoguesStore = defineStore('dialogues', () => {
  ...
  async function loadDialogues(): Promise<void> {
    ...
    try {
      const { data } = await api.get<Paginated<DialogueListItem>>('/api/dialogues')
    ...
    try {
      ...
      const [detail, page] = await Promise.all([
        known
          ...
          : api.get<DialogueListItem>(`/api/dialogues/${id}`).then((r) => r.data),
